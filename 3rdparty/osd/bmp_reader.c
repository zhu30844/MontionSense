...
int bmp_check(FILE *pfile, BITMAPFILEHEADER *bitHead, BITMAPINFOHEADER *bitInfoHead) {
	MS_LOG_INFO("bmp_check\n");
	uint16_t fileType;
	fread(&fileType, 1, sizeof(uint16_t), pfile);
	if (fileType != 0x4d42) {
		MS_LOG_ERROR("file is not .bmp file!");
		return -1;
	}
	...
	if (bitInfoHead->biBitCount < 24) {
		MS_LOG_ERROR("bmp must be 24/32 depth!");
		return -1;
	}
	MS_LOG_INFO("bitInfoHead->biBitCount is %d\n", bitInfoHead->biBitCount);
...
int load_bmp_form_file(osd_data_s *data) {
	...
	uint8_t *bmp_data;
	MS_LOG_INFO("load_bmp_form_file\n");
...
void save_argb8888_to_bmp(unsigned char *buffer, int buf_w, int buf_h) {
	...
	MS_LOG_DEBUG("buf_w is %d, buf_h is %d, filesize is %d\n", buf_w, buf_h, filesize);
	FILE *f;
