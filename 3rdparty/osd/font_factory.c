...
int create_font(const char *font_path, int font_size) {
	...
	if (!library_) {
		MS_LOG_ERROR("FT_Init_FreeType fail\n");
		pthread_mutex_unlock(&g_font_mutex);
		return -1;
	}
	FT_New_Face(library_, font_path, 0, &face_);
	if (!face_) {
		MS_LOG_ERROR("please check font_path %s\n", font_path);
...
void draw_argb8888_wchar(unsigned char *buffer, int buf_w, int buf_h, const wchar_t wch) {
	pthread_mutex_lock(&g_font_mutex);
	if (!face_) {
		MS_LOG_INFO("please check font_path %s\n", *font_path_);
		pthread_mutex_unlock(&g_font_mutex);
		return;
	}
	FT_Error error;
	...
	if (error) {
		MS_LOG_DEBUG("FT_Load_Char error\n");
		pthread_mutex_unlock(&g_font_mutex);
		return;
	}
	// DrawYuvMapBuffer(buffer, buf_w, buf_h);
	// MS_LOG_DEBUG("slot_->bitmap.rows is %d\n", slot_->bitmap.rows);
	// MS_LOG_DEBUG("slot_->bitmap.width is %d\n", slot_->bitmap.width);
	// MS_LOG_DEBUG("slot_->bitmap.pitch is %d\n", slot_->bitmap.pitch);
	// MS_LOG_DEBUG("slot_->bitmap.pixel_mode is %d\n", slot_->bitmap.pixel_mode);
	// MS_LOG_DEBUG("slot_->bitmap.buffer addr is %p\n", slot_->bitmap.buffer);
...
int wstr_get_actual_advance_x(const wchar_t *wstr) {
	...
	for (int i = 0; i < len; i++) {
		...
		pen_.x += slot_->advance.x;
		pen_.y += slot_->advance.y;
		// MS_LOG_INFO("slot_->advance.x is %d\n", slot_->advance.x);
	}
	pthread_mutex_unlock(&g_font_mutex);
